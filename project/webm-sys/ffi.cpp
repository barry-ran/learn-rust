...
extern "C" {
  ...
  const uint32_t ELEMENT_ID_MKVCLUSTE = 0x1F43B675;
  ...
  void mux_writer_set_userdata(MkvWriterPtr writer,                              
                              void* user_data) {    
    static_cast<FfiMkvWriter*>(writer)->user_data = user_data;    
  }
  ...
  const uint32_t SEGMENT_MODE_LIVE = 0x1;
  const uint32_t SEGMENT_MODE_FILE = 0x2;

  void mux_segment_set_mode(MuxSegmentPtr segment, const uint32_t mode) {
    mkvmuxer::Segment::Mode mux_mode = mkvmuxer::Segment::kFile;
    
    switch(mode) {
    case SEGMENT_MODE_LIVE: mux_mode = mkvmuxer::Segment::kLive; break;
    case SEGMENT_MODE_FILE: mux_mode = mkvmuxer::Segment::kFile; break;
    default: mux_mode = mkvmuxer::Segment::kFile;
    }
    segment->set_mode(mux_mode);
  }
