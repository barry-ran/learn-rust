fn main() {
    ...
    */
    ...
    进一步讨论，为什么使用继承顺序Fn>FnMut>FnOnce来达到上面的规则，而不是相反的继承来达到Fn类型的参数可以传递FnOnce？
    其实还是从这三个Fn对环境的影响来推理：
    我设置了一个闭包参数类型是FnOnce，那我可以接受这个闭包对环境的影响是获取所有权，并且我只会调用这个闭包一次，那我传一个Fn进来也是
    可以接受的，因为Fn对环境的影响更小，而且可以调用多次（虽然我只调用一次）
    那如果反过来：
    我设置了一个闭包参数类型是Fn，那么我的期望是这个闭包最多会获取环境变量的不可变引用，并且可以多次调用，如果传递一个FnOnce进来，
    它会获取环境变量的所有权，并且只能调用一次，不符合我的期望。    
    */
